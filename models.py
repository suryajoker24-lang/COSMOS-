# Unified Scientific Candidate & Star Result Data Models
import numpy as np
...
class TransitCandidate:
    """Unified scientific transit candidate object with physical and vetting parameters."""
    star_id: str
    candidate_id: int = 1
    source_method: str = "BLS"
    period: float = 0.0          # days
    t0: float = 0.0              # BJD epoch
    ...
    def to_dict(self) -> Dict[str, Any]:
        d = asdict(self)
        # Convert numpy arrays to lists if serializing
        if self.folded_phase is not None and isinstance(self.folded_phase, np.ndarray):
            d["folded_phase"] = self.folded_phase.tolist()
        if self.folded_flux is not None and isinstance(self.folded_flux, np.ndarray):
            d["folded_flux"] = self.folded_flux.tolist()
        if self.tls_model is not None and isinstance(self.tls_model, np.ndarray):
            d["tls_model"] = self.tls_model.tolist()
        return d

@dataclass
class StarDetectionResult:
    """Complete characterisation result for a single star."""
    star_id: str
    prediction: int = 0
    confidence: float = 0.0
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "period": round(self.period, 6) if self.period is not None else None,
            "depth_ppm": round(self.depth_ppm, 2) if self.depth_ppm is not None else None,
            "duration_hours": round(self.duration_hours, 4) if self.duration_hours is not None else None,
            "status": self.status,
